...
public class canvasEvent : BasePanel
{
    ...
    protected override void OnClick(string buttonMame)
    {
        ...
        switch (buttonMame)
        {
            ...
            case "changeButton2":
                // JsonDataManager.getInstance();
                deviceModel.getInstance().updateDeviceInfo("123321123321123$2$2$2", "deviceStatus", "5");
                break;

            case "changeCameraPosition":
                // deviceModel.getInstance().updateDeviceInfo("123321123321123$2$2$2", "deviceStatus", "5");
                EventCenterOptimize.getInstance().EventTrigger<string>("changeCameraPosition", "string");
                break;
            case "perspective":
                EventCenterOptimize.getInstance().EventTrigger<string>("perspective", "string");
                break;
            case "backBtn":
                Debug.Log("backBtn");
