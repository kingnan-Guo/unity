Shader "Unlit/tangentSpaceNormalMapping"
{
    Properties
    {
        ...
        _BumpScale("BumpScale", Range(0, 1)) = 1 // 法线贴图缩放； 用于控制 凹凸程度
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            v2f vert (appdata_tan v)
            {
                ...
                // 1、求副切线向量 
                // cross 叉乘; v.tangent.w 切线方向； v.normal 是模型空间的法线； v.tangent.xyz 是模型空间的切线
                float3 binormal = cross(normalize(v.normal), normalize(v.tangent.xyz)) * v.tangent.w;// 副切线向量
                // 2、旋转 矩阵，因为只做了旋转； 使用 roation 矩阵，把世界空间的光照方向 转换成 切线空间
                float3x3 roation = float3x3(normalize(v.tangent.xyz), binormal, normalize(v.normal));

                // 使用 TANGENT_SPACE_ROTATION 矩阵，把世界空间的光照方向 转换成 切线空间; 可以替换上面的 1、2
                // 求切线空间视角及 光源方向 
                o.lightDir = mul(roation, ObjSpaceLightDir(v.vertex)).xyz;// ObjSpaceLightDir 将光照转换成模型空间； 再乘以 旋转矩阵转到切线空间
                o.viewDir = mul(roation, ObjSpaceViewDir(v.vertex)).xyz; // ObjSpaceViewDir 将视角转换成模型空间； 再乘以 旋转矩阵转到切线空间




                // // 2、求切线空间到世界空间的矩阵
                // float3x3 tangentToWorld = float3x3(normalize(v.tangent.xyz), binormal, normalize(v.normal));
                // // 3、把世界空间的光照方向 转换成 切线空间
                // o.worldNormal = mul(tangentToWorld, worldNormal);
